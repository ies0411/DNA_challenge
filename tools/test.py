...
class DemoDataset(DatasetTemplate):
    def __init__(
        ...
        self.sample_file_list = sorted(
            [
                os.path.join(root_path, f"points/{frame_id}{ext}")
...
def parse_config():
    ...
    parser.add_argument(
        "--cfg_file", type=str, required=False, help="Specify the config for demo"
    )
    parser.add_argument(
        "--data_path", type=str, required=False, help="Specify the custom_av directory"
    )
    parser.add_argument(
        "--ckpt", type=str, required=False, help="Specify the pretrained model"
    )
    parser.add_argument(
        "--ext",
        type=str,
    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(tqdm(demo_dataset, desc="Processing dataset")):
            ...
            if num_obj == 0:
                ...
                assert (
                    num_obj != 0
                ), "No objects detected. The program requires at least one object to be detected."
